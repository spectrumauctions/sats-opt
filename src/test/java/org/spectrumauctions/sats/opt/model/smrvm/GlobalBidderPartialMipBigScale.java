...
public class GlobalBidderPartialMipBigScale {
    ...
    public void test() {
        MIP mip = new MIP();
        SimplifiedMultiRegionModel model = new SimplifiedMultiRegionModel();
        model.setNumberOfGlobalBidders(3);
        model.setNumberOfLocalBidders(0);
        model.setNumberOfRegionalBidders(0);
        Collection<SMRVMBidder> bidders = model.createNewPopulation(5413646L);
    ...
    public void testWirAndWi(){
        ...
        model.setNumberOfRegionalBidders(0);
        Collection<SMRVMBidder> bidders = model.createNewPopulation(5413646L);
        SMRVMWorld world = bidders.iterator().next().getWorld();
        double scalingFactor = SMRVM_MIP.calculateScalingFactor(bidders);
        double biggestScaledValue = SMRVM_MIP.biggestUnscaledPossibleValue(bidders).doubleValue() / scalingFactor;
        WorldPartialMip worldPartialMip = new WorldPartialMip(bidders, biggestScaledValue, scalingFactor);
